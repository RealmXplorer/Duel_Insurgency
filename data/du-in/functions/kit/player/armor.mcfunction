execute unless entity @s[scores={steveSkin=1..}] run function du-in:kit/player/skins/default
execute if entity @s[scores={steveSkin=1}] run function du-in:kit/player/skins/alex
execute if entity @s[scores={steveSkin=2}] run function du-in:kit/player/skins/herobrine
execute if entity @s[scores={steveSkin=3}] run function du-in:kit/player/skins/gonersteve
execute if entity @s[scores={steveSkin=4}] run function du-in:kit/player/skins/tplayer
execute if entity @s[scores={steveSkin=5}] run function du-in:kit/player/skins/sbplayer
tag @s remove armor