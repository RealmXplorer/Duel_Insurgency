...
execute unless entity @a[tag=void] if entity @a[scores={kit=13},tag=boneAttack,tag=!sabotaged] as @a[gamemode=!spectator,distance=..3,tag=playing] unless score @e[type=marker,scores={papyrusHitTimer=..29},sort=nearest,limit=1] team = @s team run damage @s 5 arrow by @a[sort=nearest,tag=boneAttack,limit=1]
execute unless entity @a[tag=void] if entity @a[scores={kit=13},tag=boneAttack,tag=!sabotaged] run damage @s 5 arrow by @a[sort=nearest,scores={kit=23},limit=1]
...
tag @s[scores={papyrusHitTimer=..0}] remove boneAttack
