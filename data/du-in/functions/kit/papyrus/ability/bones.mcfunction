...
execute unless entity @a[tag=void] if entity @a[scores={kit=13},tag=boneAttack,tag=!sabotaged] as @a[gamemode=!spectator,distance=..3,tag=playing] unless score @e[type=marker,scores={papyrusHitTimer=..29},sort=nearest,limit=1] team = @s team run damage @s 3.5 arrow by @a[sort=nearest,tag=boneAttack,limit=1]
