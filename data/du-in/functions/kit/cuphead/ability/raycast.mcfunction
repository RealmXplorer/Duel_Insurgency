...
execute as @a[gamemode=!spectator,tag=playing,distance=..1] run damage @s 4 minecraft:generic

#execute as @e run damage @s 4 minecraft:generic by @a[scores={kit=21},tag=kitActions,limit=1]


execute positioned ^ ^ ^1 if entity @s[distance=..10] run function du-in:kit/cuphead/ability/raycast
