...
execute if entity @s[tag=!sabotaged] as @a[distance=0.05..4,tag=playing,gamemode=!spectator,tag=!teamDead] unless score @s team = @a[tag=kitActions,limit=1,tag=playing,scores={kit=27}] team run function du-in:kit/puss/ability/hit
