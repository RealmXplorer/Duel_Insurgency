...
execute if entity @a[distance=.05..5.25,tag=playing,gamemode=!spectator,tag=!teamDead,gamemode=!spectator] run function du-in:kit/ralsei/ability/success

#If player is too far
execute unless entity @a[distance=.05..5.25,tag=playing,gamemode=!spectator,tag=!teamDead] run function du-in:kit/all/ability/far
