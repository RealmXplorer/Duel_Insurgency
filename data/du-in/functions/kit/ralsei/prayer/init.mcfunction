#Fail
execute if entity @s[tag=!teamMode,tag=maxTP] if entity @s[scores={Health=14..}] run function du-in:kit/ralsei/prayer/full
execute if entity @s[tag=!maxTP] run function du-in:kit/ralsei/prayer/empty
