...
execute as @a[gamemode=!spectator,distance=..1,tag=playing,nbt={Fire:-20s}] unless entity @s[scores={deathAbilityTimer=1..}] at @s unless block ~ ~ ~ water unless score @e[tag=flame,limit=1,sort=nearest] team = @s team run function du-in:kit/asgore/ability/contact
