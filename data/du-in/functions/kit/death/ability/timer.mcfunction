...
execute unless entity @a[tag=void] as @a[gamemode=!spectator,distance=..2,tag=playing,nbt={Fire:-20s}] unless entity @s[scores={deathAbilityTimer=1..}] at @s unless block ~ ~ ~ water unless score @e[type=block_display,tag=flame,limit=1,sort=nearest] team = @s team run function du-in:kit/death/ability/contact
