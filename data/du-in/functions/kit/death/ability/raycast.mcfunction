...
execute as @e[type=block_display,tag=flame,tag=notSet] run execute store result score @s team run scoreboard players get @a[scores={kit=14},tag=kitActions,sort=nearest,limit=1] team
...
execute positioned ^-.3 ^ ^2 if entity @s[distance=..25] run function du-in:kit/death/ability/raycast
