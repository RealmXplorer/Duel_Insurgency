...
# Gamemode Select Inventory Items #
    ...
    # Credits Item # (FUNCTIONALITY THROUGH ITEM FUNCTION)
        ...
        execute if entity @s[tag=!parkour,tag=!credits,tag=!shop,gamemode=adventure,tag=!working,nbt=!{Inventory:[{tag:{fixedItem:1b},id:"minecraft:carrot_on_a_stick",Slot:17b}]}] run function du-in:lobby/item/credits
