    ...
    # Spam Mode Item # (FUNCTIONALITY RUNS THROUGH ITEM FUNCTION)
        execute if entity @s[nbt=!{Inventory:[{tag:{fixedItem:1b},id:"minecraft:cookie",Slot:24b}]},tag=!working,gamemode=!creative,tag=!subLobby] run function du-in:lobby/item/spam

    # Random Mode Item # (FUNCTIONALITY RUNS THROUGH ITEM FUNCTION)
        execute if entity @s[nbt=!{Inventory:[{tag:{fixedItem:1b},id:"minecraft:rabbit_foot",Slot:20b}]},tag=!working,gamemode=!creative,tag=!subLobby] run function du-in:lobby/item/random_mode

    # Timed Mode Item # (FUNCTIONALITY RUNS THROUGH ITEM FUNCTION)
        execute if entity @s[nbt=!{Inventory:[{tag:{fixedItem:1b},id:"minecraft:iron_ingot",Slot:22b}]},tag=!working,gamemode=!creative,tag=!subLobby] run function du-in:lobby/item/timed_mode