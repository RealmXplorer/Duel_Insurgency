...
execute as @a[gamemode=!spectator,tag=playing,tag=!startgame,tag=!chungus] unless entity @s[scores={palpTimer=-99..}] unless entity @s[scores={Acount=1}] unless entity @s[level=3..] run xp set @s[tag=!grave,tag=!poisonApple,tag=!unicornBow,tag=!midasHand] 2 levels
