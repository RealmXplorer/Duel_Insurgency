...
#execute store result bossbar bossbar:koth value run scoreboard players get @p[distance=..2,tag=!gasterInvisible,tag=!win,tag=!lose,tag=!winend] kothTimer
...
execute as @a[tag=playing,distance=..2,tag=!gasterInvisible,tag=!win,tag=!lose,tag=!winend] unless entity @s[scores={floweyHitTimer=0..}] run scoreboard players add @s kothTimer 1
