...
execute if block ~ ~ ~ #du-in:zomb_invalid_air rotated as @s run tag @s add airBlock

execute if entity @s[tag=teamMode] if entity @e[type=marker,tag=gameObject,distance=..10] rotated as @s run tag @s add airBlock

execute if entity @s[tag=kothIngame] if entity @e[type=interaction,tag=gameObject,distance=..10] rotated as @s run tag @s add airBlock

execute if blocks ~ ~ ~ ~ ~.5 ~ ~ ~-.5 ~ all run tag @s add airBlock
