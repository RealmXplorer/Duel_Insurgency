...
execute if entity @s[tag=!empower] as @a[gamemode=!spectator,tag=playing,distance=..1.5,sort=nearest,limit=1,tag=!dartShooter] at @s run function du-in:kit/judy/ability/hit
execute if entity @s[tag=empower] as @a[gamemode=!spectator,tag=playing,distance=..1.5,tag=!dartShooter] at @s run function du-in:kit/judy/ability/hit
