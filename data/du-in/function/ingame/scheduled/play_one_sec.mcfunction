...
execute if entity @s[tag=!pussFear,tag=!kratosRage,tag=!kitMenu,tag=!teamDead,tag=!noMainWeapon,tag=!pawbertInvisible,tag=!gasterInvisible,tag=!asrielSaber,tag=!thrown,tag=!brokenShield] unless entity @s[scores={floweyHitTimer=0..}] unless entity @s[scores={skeletonMode=1..}] run function du-in:kit/all/weapon/test
