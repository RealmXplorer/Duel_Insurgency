...
execute if entity @a[scores={LEVEL_OF_FUN=5..}] run scoreboard players set #main FRIEND 0
execute as @a[tag=ingame,scores={LEVEL_OF_FUN=5..,kit=0..}] store result score #main FRIEND run return run function du-in:lobby/team_select/item/sort
