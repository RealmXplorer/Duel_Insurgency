...
execute if score #main pylonsDestroyed matches 3.. run item replace entity @s hotbar.4 with minecraft:carrot_on_a_stick[item_model="du-in:lobby/void_play",custom_name='{"text":"RUN","color":"green","bold":true,"italic":false}',lore=['{"text":"Right click to use!","color":"dark_purple"}'],custom_data={du-in:'playItem'},consumable={consume_seconds:0,animation:"none",sound:"block.candle.hit",has_consume_particles:false}]
